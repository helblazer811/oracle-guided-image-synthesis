...
import util
from loss.learned_masked_vae_triplet_loss import LearnedMaskedVAETripletLoss
...
class LearnedMaskedVAE(nn.Module):

    def __init__(self, z_dim, in_shape, layer_count=4, channels=1, d=128, loss_name="LearnedMaskedVAETripletLoss", kl_beta=1.0, triplet_beta=1.0, triplet_margin=1.0, num_triplets=None, mask_regularization_beta=0.1, attributes=[2, 3]):
        super(LearnedMaskedVAE, self).__init__()
        self.d = d
        self.z_dim = z_dim
    ...
    def _setup_loss_function(self):
        if self.loss_name == "LearnedMaskedVAETripletLoss":
            self.loss_function = LearnedMaskedVAETripletLoss(kl_beta=self.kl_beta, triplet_beta=self.triplet_beta, latent_dim=self.z_dim, attributes=self.attributes)
        else:
