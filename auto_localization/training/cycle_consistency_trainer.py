...
class CycleConsistencyTrainer():
    ...
    def from_config(cls, config, data_manager=None, model=None):
        return cls(
            ...
            adam_beta_1 = config["model_config"]["adam_beta_1"],
            adam_beta_2 = config["model_config"]["adam_beta_2"],
            ...
            cycle_consistent_warmup = config["cycle_consistent_warmup"] if "cycle_consistent_warmup" in config else 1.0, 
            cycle_consistent_beta = config["model_config"]["cycle_consistent_beta"] if "cycle_consistent_beta" in config else 1.0,
    ...
    def _setup_optimizers(self):
        ...
        self.auto_encoder_optimizer = torch.optim.Adam(
            self.model.parameters(),
            betas=(self.adam_beta_1, self.adam_beta_2),
            lr=self.lr
        )

        self.reverse_cycle_optimizer = torch.optim.Adam(
            self.model.get_encoder_parameters(),
            betas=(self.adam_beta_1, self.adam_beta_2),
            lr=self.lr
        ) # todo tune adam parameters

    """
        Sets up a learning rate scheduler
    """
    def _setup_schedulers(self, scheduler_type="exponential"):
        if scheduler_type is "exponential":
            self.auto_encoder_scheduler = torch.optim.lr_scheduler.ExponentialLR(
                self.auto_encoder_optimizer,
                gamma = self.gamma,
            )

            self.reverse_cycle_scheduler = torch.optim.lr_scheduler.ExponentialLR(
                self.reverse_cycle_optimizer,
                gamma = self.gamma,
            )
    ...
    def test_epoch(self):
        ...
        for key in loss_dict.keys():
            wandb.log({"test_"+key:loss_dict[key]})
    ...
    def train_epoch(self, epoch=1):
        ...
        for (recon_data, triplet_data) in tqdm(zip(recon_iter, triplet_iter)): 
            ...
            mean, log_var, latent, recon = self.model(x)
            # auto encoder loss
            autoencoder_loss_dict = self.model.loss_function.auto_encoder_loss(x, recon, mean, log_var)
            loss = autoencoder_loss_dict["loss"]
            loss.backward()
            ...
            triplet_loss = self.model.loss_function.triplet_beta * self.model.loss_function.triplet_loss(anchor[0], positive[0], negative[0])
            triplet_loss.backward(retain_graph=True)
            # Compute the cycle consistency loss
            cycle_consistent_loss = self.model.loss_function.cycle_consistent_beta * self.model.loss_function.pairwise_cycle_consistent_loss(self.model, x)
            cycle_consistent_loss.backward()
            # Compute percentage satisfied
            triplet_percentage = self.model.loss_function.triplet_percentage(anchor[0], positive[0], negative[0])
            """
            kwargs = {"model":self.model}
            reverse_cycle_dict = self.model.loss_function.reverse_cycle_loss(x, recon, mean, log_var, triplet_data, **kwargs)
            loss = reverse_cycle_dict["loss"]
            loss.backward()
            ...
            loss_dict = {
                "loss": autoencoder_loss_dict["loss"],
                "recon_loss": autoencoder_loss_dict["recon_loss"],
                "kl_divergence_loss": autoencoder_loss_dict["kl_divergence_loss"],
                "triplet_loss": reverse_cycle_dict["triplet_loss"],
                "triplet_percentage": reverse_cycle_dict["triplet_percentage"],
                "cycle_consistent_loss": reverse_cycle_dict["cycle_consistent_loss"]
            }
        ...
        for key in loss_dict.keys():
            wandb.log({"train_"+key:loss_dict[key]})
