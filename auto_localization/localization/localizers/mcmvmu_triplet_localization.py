...
"""
class MCMVMUTripletLocalizer(ActiveLocalizer):
    ...
    """
        Computes the expected certainty of a given  
    """
    def compute_expected_certainty(self, pair):
        """
            Triplet likelihood probability
        """
        def calculate_probability(muA, muP, muN, varA, varP, varN, margin = 0.0):
            ...
            return probs
        # get the mean and variances of the pair of items
        muP = torch.Tensor(self.embedding[pair[0]])
        varP = torch.Tensor(self.embedding_logvars[pair[0]]).exp()
        muN = torch.Tensor(self.embedding[pair[1]])
        varN = torch.Tensor(self.embedding_logvars[pair[1]]).exp()
        # go through each item in the emebedding and calculate its expected certainty
        expected_certainties = []
        for index in range(len(self.embedding)):
            mean = torch.Tensor(self.embedding[index])
            var = torch.Tensor(self.embedding_logvars[index]).exp()
            expected_certainty = abs(calculate_probability(mean, muP, muN, var, varP, varN, margin = 0.0).item() - 0.5)
            expected_certainties.append(expected_certainty)
        # compute the mean expected certainty
        mean_expected_certainty = np.mean(expected_certainties)
        return mean_expected_certainty

    """
        Returns the optimal mean cut max variance query
        from a chosen sample
    """
    def get_mcmvmu_query(self, sample, W_mean, W_cov):
        # scale the embedding
        ...
        for j in range(self.Npairs):
            ...
            (A_emb, tau_emb) = pair2hyperplane(pair, scaled_embedding, self.normalization)
            varest = np.dot(A_emb, W_cov).dot(A_emb)
            distmu = np.abs(
                (np.dot(A_emb, self.mu_W) - tau_emb)
                / np.linalg.norm(A_emb)
            )
            # calculate the expected certainty
            expected_certainty = self.compute_expected_certainty(pair)
            wandb.log({"expected_certainty": expected_certainty})
            wandb.log({"varest": self.k * np.sqrt(varest)})
            wandb.log({"distmu_mcmv": self.lambda_pen_MCMV * distmu})
            # choose highest variance, but smallest distance to mean
            mcmv_values[j] = self.k * np.sqrt(varest) - self.lambda_pen_MCMV * distmu + self.lambda_latent_variance * expected_certainty
        # choose the best one
        best_query_index = np.argmax(mcmv_values)
        optimal_query_pair = sample[best_query_index] 
        index_a, index_b = optimal_query_pair
    ...
    def get_query(self, sample_from_existing=True):
        ...
        optimal_query_pair = self.get_mcmvmu_query(random_query_pairs, W_mean, W_cov)
